...
[SCOPE_HINT] — Files, packages, or functions mentioned in the task, extracted in Step 1.2
...
- **Clean**: proceed.
- **Dirty**: Read `./bug-dirty-tree.md`. Return here after verified task-owned work is recovered.

Use the initial baseline pinned by `session.md`; the bug baseline is another name for that same commit. It does not advance during bug resolution. On resume, verify any existing tag still points there. A missing expected tag is a blocker.

```bash
git tag "bug/$TASK_KEY/baseline" "implement/$TASK_KEY/baseline"  # first bug entry only
```

### 1.2 Read Task and Extract Context

Read `TASK_STATE/TASK.md` and the session/progress records. Extract `[BUG_DESCRIPTION]`, `[SCOPE_HINT]`, and any verbatim error messages or stack traces. Record the reproduction phase. If a valid committed reproduction already exists, verify it fails at its recorded reproduction ref and resume Step 2 rather than creating another reproducer.

Based on the task's shape:
- **Describes a capability gap** (language like "add", "implement", "support", or an inability to fill in "[Expected] but [Actual]" because no actual behavior exists): This is a feature request. Record the triage in `progress.md` and read `./plan.md`.
...
Enumerate all plausible root causes based on task content, error messages, and stack traces. For each hypothesis, state:
- What could cause this behavior.
- Which code paths or data-flow segments are implicated.
- What evidence supports or contradicts it.

If every hypothesis reduces to "the described behavior is not implemented anywhere," record the finding in `progress.md` and read `./plan.md`.
...
On divert, record the disqualifier and why no outcome-level test fits, then read `./plan.md` (planning chooses an outcome guard where meaningful). Otherwise proceed to Step 1.5.
...
- **One or more tests fail**: Commit the failing tests in one reproduction commit, tag the state, capture `[TEST_FAILURE_OUTPUT]`, and save each viable pathway/test in `TASK_STATE/progress.md`. Proceed to Step 2. Never bypass a hook to commit deliberately failing tests; if repository requirements prevent this reproduction checkpoint, preserve the tests/evidence and report the conflicting requirement through `./blocked.md`.
...
- **No tests fail, hypotheses exhausted**: Save attempted hypotheses and evidence, then read `./blocked.md`.
...
git tag -f "bug/$TASK_KEY/reproduction" HEAD
```
...
<scope-rules>
**Non-deterministic bugs**: If `[TEST_FAILURE_OUTPUT]` lacks a source file and line location (for example, timeout or race condition), data-flow tracing may not be possible. Document what is known and treat the data-flow path as a hypothesis rather than a trace.
</scope-rules>
...
- **Reproduction test still fails, or validation fails on implementation grounds**: Treat as NEEDS_REVISION. Save attempt evidence, revert only this attempt's verified source changes, preserve the reproducer, and retry. If three attempts are exhausted, save the specific obstacle and read `./blocked.md`.
- **Cannot proceed** (structural obstacle): Save the reasoning and exact failed prerequisite, then read `./blocked.md`.
...
For NEEDS_REVISION rollback, inspect the diff and restore the exact task-owned source files changed by this attempt. Preserve reproduction tests, supplied work and task state. Remove only recorded scratch files; never use a broad clean to erase an uncommitted reproducer.
...
1. Revert any non-test source changes made since `bug/$TASK_KEY/reproduction`.
2. Run the corrected test.

Based on the corrected test's result:
- **Fails (valid correction)**: Commit the corrected test, update the tag (`git tag -f "bug/$TASK_KEY/reproduction" HEAD`), capture new `[TEST_FAILURE_OUTPUT]`, and return to Step 2.2: Fix.
- **Passes (invalid correction)**: Revert the test correction. If resolve attempts remain, return to Step 2.2; otherwise report the test-validation failure through `./blocked.md`.
- **Corrections exhausted (2 already applied)**: Record why the reproduction became unreliable and read `./blocked.md`.
...
- **Unresolvable error**: Save the exact failure output and read `./blocked.md`; do not retry hoping it resolves itself.
...
Follow `./worktrees.md` to create and provision a detached worktree at the pinned bug baseline or reproduction ref. Run the discriminating test before accepting a correction, capture the evidence, then remove that exact temporary worktree. Never switch branches in the active checkout to perform this check.
