...
For new behavior whose contract is worth validating ahead of implementation, consult `$cardless:tdd-bootstrap` and structure implementation along its three phases. Skip bootstrap for refactors, spikes, visual work, glue code, one-shot scripts, framework-determined shapes and small in-place edits.
...
For each testable uncertainty load `$cardless:spike`. Independent spikes can run concurrently in separate directories. Revise the plan using the results. A disproven load-bearing assumption invalidates the plan from intent through approach: rewrite it rather than patching around the false premise.
