...
Run lint and typecheck plus each package suite the `implement/[TASK_KEY]/baseline..HEAD` diff touches (or the plan's validation commands, when no plan exists).
...
Diff `implement/[TASK_KEY]/baseline..HEAD`, then read the task (and plan, if one exists) again with that diff in hand. If you implemented this yourself or the diff is large, dispatch a fresh-eyes subagent per angle.
...
**Delivered experience.** Read the task's acceptance criteria and exercise the implementation from its user-facing entry points. Does it deliver what the task asked for?

Each finding names the mechanism to remove — the wrong rule, axis, or key — and the witness configurations to pin in-suite.
...
Reached only once Step 3 completes with no findings. Record the reviewed HEAD, angles, acceptance evidence and validation results under `TASK_STATE/reports/`, then update `session.md`. Return control to the caller. Any new commit invalidates that review until its effects have been rechecked.
