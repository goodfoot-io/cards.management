...
`cards` is a plugin-provided executable on `PATH` covering get, create, list, search, variable-group discovery, attach, action, and watch. Invoke it directly as a bare command.
...
**List variable groups** — Discover the groups available for action dispatches in a workspace:
```
cards variable-group list
cards variable-group list --workspace-path <path>
cards variable-group list --jsonpath <expr>
```
Each summary contains `id`, `name`, optional `description`, `variableCount`, and `secretCount`. The command never returns variable values or secrets. Use the group `id`, not its display name, when launching an action.
...
cards <card-id> action <action-id> --variable-group <id> --variable-group <id>
```
...
`--variable-group` is ordered and repeatable. When supplied, the action receives exactly those group IDs for that dispatch only; it does not change the variable-group toggles persisted in the webview. When the flag is omitted, dispatch retains the normal persisted variable-group selection.
...
```
Outcome defaults to `success`. Exit 0 confirms durable acceptance by the authenticated runtime; the handler terminates the session gracefully and the card still settles on `needs_review`. Without protected runtime credentials the command exits non-zero — it only works inside an admitted action.
...
Use `--workspace-path` only if the user explicitly requests operating on a different repository, including variable-group discovery for that repository.
...
- **archived**: This card has been archived and is no longer in the active workflow. Before setting a card to this status, add a comment (see Adding a Comment) recording the retirement reason — why the card is being retired rather than carried forward as done, what evidence was reviewed, and what disposition (if any) applies to its current description/notes. Write it so a reader who only sees the archived card understands why, without relying on prior comment history.
</card-status>
