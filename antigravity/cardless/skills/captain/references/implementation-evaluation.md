...
Run lint and typecheck plus each package suite the `implement/[TASK_KEY]/baseline..HEAD` diff touches (or the plan's validation commands).
...
Diff `implement/[TASK_KEY]/baseline..HEAD`, then read the task (and plan, if any) again with that diff in hand. If you implemented this yourself or the diff is large, dispatch a fresh-eyes subagent per angle.
