...
**Purpose**: The `adhoc-cleanup` daemon records the agent PID and monitors it. On PID death it requests `needs_review` through the Cards API. If that request fails, teardown fails closed and retains the ref for explicit recovery; no hook edits or commits card metadata directly.
...
- Card repository structure (plans, comments, attachments) → `cards` skill
