...
For on-this-machine workers, use `notify_when_idle: true` only when the live `SendMessage` schema advertises it. It is a one-shot notice, not a cross-machine subscription. Use completion notifications/the host's supported event wait and process every message; an idle notification without a protocol report is not success. While a worker has an outstanding assignment, set the five-minute progress check with the host's scheduler (`CronCreate`) and clear it at drain. Do not infer unlimited concurrency from one successful dispatch: respect declared limits, keep the roster reviewable, and queue work when capacity is unknown or exhausted. Do not busy-poll session listings.
