...
Read the repository's CLAUDE.md, any applicable nested instructions, and its declared setup and validation commands. Record host restrictions, required attribution on GitHub posts, and the user's authorized delivery actions. The checkout must already be on a dedicated task branch. Keep a cloud host's assigned branch: its Git proxy may allow pushes only to that branch. The caller's dispatch branch need not be knowable from this checkout; use the explicit host assignment and actual Git state, not a guessed relationship between branch names. For a local run, prepare the dedicated branch before invoking this workflow. A detached HEAD or a checkout on the PR base is a setup blocker.
