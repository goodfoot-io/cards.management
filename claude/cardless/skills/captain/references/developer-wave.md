...
Name `cardless:developer` and `cardless:tdd-bootstrap` explicitly in the dispatch. Include the literal task state/scratch paths and the commit/Markdown conventions from `session.md` in the brief; workers must not assume inherited context or mutate the lead's session record.
