...
|--------|-----------|---------|
| `stream-sync-watcher` | SessionStart/`EnterWorktree` hooks | Remains in the spawning session, monitors agent PID, and finalizes transcript capture on stop or signal |
| `adhoc-cleanup` | `EnterWorktree` hook | Monitors agent PID, transitions card to `needs_review` on session end |
| Branch cleanup watcher | Extension durable branch-cleanup effect | Runs the installed worker with server-derived environment parameters and awaits its exit; failure remains a failed effect |
...
**Win32 transcript/adhoc spawns**: Resolve `node.exe` directly (not through a `.cmd` shim) via `resolveDetachedNodeInterpreter()` with `windowsHide: true`. The stream-sync watcher stays in the spawning session; adhoc cleanup is detached. The branch-maintenance effect separately invokes the installed worker shim and awaits its exit.
